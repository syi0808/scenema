...
- 프리뷰는 Hero를 벗어나면 실행을 중단해 부모 페이지의 스크롤 위치를 바꾸지 않는다.
- 프리뷰 내부 Actorble 커서는 Scenema 오버레이와 카드보다 위에 표시한다.
...
- 대상: Code의 `DOM action` 탭
- 행동: 사용자가 진행하면 Actor가 실제 탭 클릭
- 결과: 코드 패널이 DOM action 예제로 변경
- 목적: 첫 단계부터 실제 커서 이동과 인터페이스 변화를 보여줌

### Step 2. 연속 DOM 동작

- 대상: Code의 `Navigation` 탭
- 행동: 사용자가 진행하면 Actor가 두 번째 탭 클릭
- 결과: 코드 패널이 Navigation 시나리오로 변경
- 목적: 한 번의 효과가 아니라 다음 타깃으로 이어지는 흐름을 증명

### Step 3. 결과 확인

- 대상: 변경된 코드 패널
- 행동: 없음
- 목적: 앞선 Actor 동작과 선언된 시나리오를 연결
