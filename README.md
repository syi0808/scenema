...
The built-in choreography uses perceptible pacing by default: pointer movement follows an `ease-in-out` curve for `800ms`, clicks approach over `1000ms` and hold for `240ms`, and typing waits `100ms` between characters. Override individual values through `actorble.actionDefaults`, or set `actorble.motion` to `false` when pointer motion should be disabled.
