...
The built-in choreography uses perceptible pacing by default: after the user proceeds, automated clicks wait `300ms` before starting; pointer movement follows an `ease-in-out` curve for `800ms`; clicks approach over `1000ms` and hold for `240ms`; typing clicks its target to focus it, holds for `240ms`, and then waits `100ms` between characters. The Actorble cursor is rendered at `2x` its standard size for visibility. Override the initial pause with `clickDelay`, override individual action values through `actorble.actionDefaults`, provide a custom `actorble.visualLayer` for different cursor rendering, or set `actorble.motion` to `false` when pointer motion should be disabled.
